...
client = OpenAI(api_key=settings.OPENAI_API_KEY) if hasattr(settings, 'OPENAI_API_KEY') else OpenAI(api_key=os.getenv('OPENAI_API_KEY'))
...
@verificado_required
@rol_requerido(['PACIENTE', 'FAMILIAR'])
def asistente_virtual_view(request):
    return render(request, 'asistente_virtual.html')
